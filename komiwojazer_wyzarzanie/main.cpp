...
std::vector<std::vector<int>> macierzOdleglosciTsplib(const std::string& filename) {
    ...
    // Szukaj sekcji współrzędnych wierzchołków
    ...
    if (!nodeCoordSection) {
        std::cerr << "Błąd: Brak sekcji NODE_COORD_SECTION w pliku " << filename << std::endl;
        exit(EXIT_FAILURE);
    }

    std::vector<std::vector<int>> odleglosci(dimension, std::vector<int>(dimension, 0));

    // Wczytaj współrzędne i oblicz odległości między miastami
    int city, x, y;
    while (file >> city >> x >> y) {
        odleglosci[city - 1][city - 1] = 0; // Odległość miasta do samego siebie
        for (int i = city; i < dimension; ++i) {
            int distance = std::round(std::sqrt(std::pow(x - odleglosci[i][0], 2) + std::pow(y - odleglosci[i][1], 2)));
            odleglosci[city - 1][i] = distance;
            odleglosci[i][city - 1] = distance;
        }
    }
...
std::vector<int> simulatedAnnealing(const std::vector<std::vector<int>>& odleglosci, double initialTemperature, double coolingRate, int maxIterations) {
...
int main() {
    ...
    for (int n : liczbaMiast) {
        ...
        std::vector<int> saSolution = simulatedAnnealing(odleglosci, 4000.0, 0.99, 15000);
